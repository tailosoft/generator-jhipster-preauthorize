...
import com.mycompany.myapp.service.mapper.RoleAuthorityMapper;
...
public class RoleAuthorityService {
    ...
    public RoleAuthorityService(RoleAuthorityRepository roleAuthorityRepository, RoleAuthorityMapper roleAuthorityMapper, UserService userService) {
    ...
    public List<RoleAuthorityDTO> findByRoleName(String roleName) {
        Set<String> currentUserAuthorities = SecurityUtils.getCurrentAuthorities();
        List<RoleAuthority> roleAuthorities = roleAuthorityRepository.findByRoleName(roleName)
            .stream().filter(ra -> currentUserAuthorities.contains(ra.getAuthority())).collect(Collectors.toList());
        return roleAuthorityMapper.toDto(roleAuthorities);
    }
}
