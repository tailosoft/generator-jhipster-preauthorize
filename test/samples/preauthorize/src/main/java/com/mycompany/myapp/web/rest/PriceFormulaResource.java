...
public class PriceFormulaResource {
    ...
    @PreAuthorize("hasAuthority('" + PRICE_FORMULA_CREATE + "')")
    public ResponseEntity<PriceFormulaDTO> createPriceFormula(@Valid @RequestBody PriceFormulaDTO priceFormulaDTO) throws URISyntaxException {
        ...
        if (priceFormulaDTO.getId() != null) {
            throw new BadRequestAlertException("A new priceFormula cannot already have an ID", ENTITY_NAME, "idexists");
        }
        PriceFormulaDTO result = priceFormulaService.save(priceFormulaDTO);
        return ResponseEntity.created(new URI("/api/price-formulas/" + result.getId()))
    ...
    @PreAuthorize("hasAuthority('" + PRICE_FORMULA_UPDATE + "')")
    public ResponseEntity<PriceFormulaDTO> updatePriceFormula(@Valid @RequestBody PriceFormulaDTO priceFormulaDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, priceFormulaDTO.getId().toString()))
            .body(result);
    }
    ...
    @PreAuthorize("hasAuthority('" + PRICE_FORMULA_READ + "')")
    ...
    @PreAuthorize("hasAuthority('" + PRICE_FORMULA_READ + "')")
    public ResponseEntity<PriceFormulaDTO> getPriceFormula(@PathVariable Long id) {
    ...
    @PreAuthorize("hasAuthority('" + PRICE_FORMULA_DELETE + "')")
    public ResponseEntity<Void> deletePriceFormula(@PathVariable Long id) {
