...
public class PriceFormulaResourceIT {
    ...
    public void getAllPriceFormulas() throws Exception {
        ...
        restPriceFormulaMockMvc.perform(get("/api/price-formulas?sort=id,desc"))
    ...
    public void getPriceFormula() throws Exception {
        ...
        restPriceFormulaMockMvc.perform(get("/api/price-formulas/{id}", priceFormula.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(priceFormula.getId().intValue()))
    ...
    private void defaultPriceFormulaShouldBeFound(String filter) throws Exception {
        restPriceFormulaMockMvc.perform(get("/api/price-formulas?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(priceFormula.getId().intValue())))
            .andExpect(jsonPath("$.[*].max").value(hasItem(DEFAULT_MAX)))
        ...
        restPriceFormulaMockMvc.perform(get("/api/price-formulas/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void updatePriceFormula() throws Exception {
        ...
        PriceFormula testPriceFormula = priceFormulaList.get(priceFormulaList.size() - 1);
        assertThat(testPriceFormula.getMax()).isEqualTo(UPDATED_MAX);
    ...
    public void deletePriceFormula() throws Exception {
        ...
        restPriceFormulaMockMvc.perform(delete("/api/price-formulas/{id}", priceFormula.getId())
